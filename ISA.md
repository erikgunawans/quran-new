...
- [ ] ISC-434: `searchDalil` is wired into `handleAnswer`, so the turn's hadith grounding is non-empty. **NOT MET — blocker 1 of 2 for real hadith answers.** `worker/src/dalil.ts` is fully built (`searchDalil`, `capForDisplay`, `MAX_DISPLAY=2`, `fetchDisplayRecords`) and returns ids in exactly the `hadith-bukhari-6962` shape the guard validates, but `handleAnswer` never calls it. Probe: `grep -c searchDalil worker/src/index.ts` > 0.
- [ ] ISC-435: `SYNTHESIS_SYSTEM_PROMPT` teaches the `[H:collection:number]` marker syntax. **NOT MET — blocker 2 of 2, and the deeper one.** `web/src/answer-contract.ts` contains ZERO mentions of hadith, marker, or `H:`. The model therefore cannot emit a receipt even with a populated union, which is why the wall is unpassable independently of ISC-434. Probe: `grep -c 'H:' web/src/answer-contract.ts` > 0.
- [ ] ISC-436: the hadith pointer is verified live on prod. **FAILED 2026-08-12 — corrected from `[DEFERRED-VERIFY]`, which was wrong to record.** Deployed (worker `88e17cff`, bundle `index-C8Ur3EzZ.js` confirmed served) and probed in real Chrome on a clean thread: the app refuses correctly, but renders the OLD `silence` copy, not `hadith-defer`. No `/api/answer` request appears in the network log for that turn, so synthesis is not reaching the endpoint on this path. Two candidate causes, NEITHER established: (a) the client's 12s `TIMEOUT_MS` in `answer-live.ts` aborting before the now-slower double generation returns — a timeout is by design an absence, so it renders silence; (b) something routing the question away from the synthesis branch before the fetch. The same question DID author an answer on the previous bundle (`CaAavd7d`), and the only client-side difference is the guard, which points at (a) — pointing at is not evidence and is not recorded as one. Needs the worker's timing and instrumented runs.
...
- [ ] ISC-440: the intermittency is explained and closed. **PARTLY.** The silence-vs-answer flip on one question was read as a caching problem for two sessions; the real cause is which verb the model reaches for, so the outcome was genuinely nondeterministic. Closed for the four measured verbs; NOT closed in general, because the verb list is still an enumeration and the next unlisted synonym leaks the same way. A construction-level test (attribution grammar, not vocabulary) is the real fix and is not built.
