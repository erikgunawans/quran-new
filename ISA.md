...
- [ ] ISC-642: the late-path `hadith-defer` substitution REPLACES a real answer the reader is already holding, and it is a live ISC-534 violation this session did not fix. **NOT MET — found, recorded, and left for Erik because it is a product call, not a bug.** `applyAi` returns `{kind:"hadith-defer"}` for a `bad_hadith` block, which makes `composed` truthy, so `main.ts` takes the fast answer off the screen at ~25 s and replaces it with the Hadis pointer. On the FAST path that is correct — no answer was ever painted. On the LATE path the reader has had a cited, grounded answer for sixteen seconds and it is taken away. The tension is real in both directions and that is why it is not being ruled on here: the pointer is the one refusal this app considers TRUE (`ONLY the hadith rule earns the Hadis pointer, because only for it is the pointer true`), and annotating instead would keep the answer but lose the pointer. **Deliberately NOT changed in the same pass that built the annotation channel** — that channel only ever fires where the code previously did nothing, so it cannot have introduced this, and folding a behaviour change into it would have made the diff impossible to reason about. Needs Erik: *does a real answer on screen outrank the Hadis pointer?*
- [ ] ISC-647: the annotation is verified on a LIVE refused turn on prod, sampled across the whole window rather than at settle. **NOT MET — deploy is Erik's, and the probe shape is already known.** The turn must be slower than `FAST_ANSWER_MS`, must end in a wall refusal, and must report `gen.reason:"blocked"` rather than `"deadline"` — the same conjunction that made ISC-537 unprobeable from a curl, since grounding must hash-verify as verbatim-ours. Drive it through Interceptor and sample the DOM at 2 s across the whole window, never at settle: `fallthrough-lane-flashes` measured a lane that renders at T+12 s and is wiped at T+16 s, invisible to a settle-time read. **Two things this session could not see and must be checked together:** whether the annotation actually paints, and whether `gen.reason` on a real refused turn is `"blocked"` at all — if production refusals are dominated by `deadline`, this channel is correct and near-silent, which is a different finding from it being broken.
