...
- [ ] ISC-323.3: decide whether `worker/src/dalil.ts:272` passes `returnValues: true`. **ERIK'S CALL, opened 2026-08-18 by ISC-323.2.** For: the candidate set becomes the one offline analysis has always described, and the ISC-323 regression record enters the pool at rank 24 — inside `CANDIDATE_K = 50`, so the reranker can finally see it. Against: **~+600 ms** on every dalil turn while ISC-487 (the ~26 s wall) is open, and a 50×1024-float payload. **A better candidate pool is not by itself a better answer** — whether Muslim 154 reaches rank 1 after `voyageai/rerank-2.5` is unmeasured, and per this repo's own rule a ranking result that names a record proves nothing about what the reader gets. Measure the reranked outcome before spending the latency.
