...
- [ ] ISC-436: the hadith pointer is verified live on prod. **DEFERRED-VERIFY** — the change is built and gated behind Erik's deploy decision. Probe once deployed: `POST /api/answer` with *"apakah benar bahwa sakit itu akan menghapus dosa kita?"* returns `blocked:"bad_hadith"`, and the browser renders the `hadith-defer` turn. Follow-up: item 1 of the next handoff.
