...
- [ ] ISC-484: the widened lane produces ANSWERS rather than refusals. **NOT MET, and this is the honest state of the cycle.** On the two questions the widening was built for, the model reaches for a prophetic attribution and does not carry a resolving marker, so `bad_hadith` stops it — `bagaimana adab kepada orang tua` and `apa keutamaan sedekah` both answered cleanly BEFORE this change and now sometimes do not. Retrieval is done; the remaining work is the PROMPT half (teaching the model to cite the hadith it is handed, per rule 7 of `SYNTHESIS_SYSTEM_PROMPT`), not the retrieval half. Probe: re-run `src/eval/wall-live-probe.ts --repeat 3` and require the two `WEAK` questions to answer, not refuse.
