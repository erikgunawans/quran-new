...
- [DEFERRED-VERIFY] ISC-202.2: the classifier is unioned into `retrieve()`, keyword hits keep precedence. CODE COMPLETE + unit-tested: `retrieve()` gained an additive `modelThemes` param (keyword pass untouched, a model-only theme reaches `MIN_SCORE` and records honest `MODEL_THEME_MATCH` provenance, keyword themes still outrank via ×count); `theme-live.ts` POSTs `/api/classify` (3s timeout) → `understandThemes`; `main.ts` awaits it on the question path only, degrades to `[]`. Tests: `retrieve-model-themes.test.ts` (union/precedence/off-corpus), `theme-live.test.ts` (fetch-mocked). Full suite 492/0, client bundles. **Follow-up: F-MODEL-WIRING — deploy the app (Worker `/api/classify` already live), then probe a keyword-miss phrase ("ngerasa Tuhan udah nyerah sama aku") surfacing a verse the lexicon would have gone silent on.**
