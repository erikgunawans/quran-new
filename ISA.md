...
- [ ] ISC-323.2: explain why the live candidate set differs from the offline reproduction over the same vectors — offline cosine ran 0.51–0.59 and surfaced Muslim 154 at rank 28; live scores run 0.43–0.50 and surface a different set. Until this is understood, no offline retrieval measurement may be quoted as evidence about live behaviour. **STILL OPEN 2026-08-17 — but the probe channel is now OPEN (ISC-372) and two candidate explanations are ELIMINATED, so what remains is narrower than "unknown".** (a) *Not an embedder mismatch*: `src/okf/build-index.ts:38,109` embeds index records with `baai/bge-m3` via OpenRouter `/v1/embeddings`, and `worker/src/dalil.ts:72,107` embeds the live query with the identical model, endpoint and no instruction prefix. Same model both sides. (b) *Not an index-metric mismatch*: `wrangler vectorize get okf-hadith` reports `dimensions 1024, metric cosine` — the metric offline cosine assumes. Live run of the ISC-323 question re-confirms the spread this ISC describes: 8 hits, cosine 0.4341–0.4603, rank 1 `hadith-bukhari-540`. What survives: ANN recall over `topK` on an approximate index (an offline EXHAUSTIVE scan can reach a rank-28 record that the live index never returns), or an index/cache population difference. Next probe is a vector count of the live index against `data/okf/vectors-bge-m3.jsonl`.
- [ ] ISC-372.1: the probe command documented in `worker/wrangler.dalil-probe.toml`'s header runs from the directory that CANNOT work. Its `cd worker && bunx wrangler dev …` resolves wrangler 3.114.17; the command must run from the repo root to resolve 4.120.0. The header is what sent two sessions into an account-scope dead end — fix the header, not just the knowledge.
...
- [x] ISC-492: the Fikih section names the amal area without implying a ruling was produced. **MET 2026-08-18.** `fiqhAreaOf` supplies a doorway card (title, sub, and links to the compilers' own kitab at `#/hadis/<collection>/<book>` — there is no `#/fikih/<area>` route and this does not invent one). Live: `berapa rakaat salat duha` → **Salat**, `bagaimana cara wudu yang benar` → **Thaharah**, `nisab zakat emas` → **Zakat**, `puasa syawal` → **Puasa**. The Fikih lane still only re-ranks; `rankByFiqhArea` is applied BEFORE the cap so it can change which two show, and it can neither admit nor refuse.
- [ ] ISC-493: section search does not present its nearest matches as if they answered the question. **NOT MET — measured 2026-08-18, and the obvious fix is FORBIDDEN by the data.** Vector search always returns its top-k, so `zxqw plumbus flarn` returns 2 hadith cards and 6 references exactly as a real question does; `dalilEmptyEl` is therefore only reachable when retrieval itself fails. A rerank floor looked clean on 3 samples (REAL 0.75–0.85 vs non-real 0.19–0.41) and **collapsed at 20**: worst real is `hukum poligami` at **0.4805**, best non-real is `qqqq wwww eeee rrrr` at **0.4102** — a 0.0703 window, against a score `dalil.ts` twice documents as "NOT a correctness signal, and not comparable across questions". A threshold in that window would silence a legitimate fiqh question to suppress gibberish. Cosine separates even worse. Open for Erik: accept it as search behaviour (the box is labelled "Cari Hadits", and a search returning nearest matches is not the answer path manufacturing an answer), or re-frame the results copy as "closest in the corpus".
- [ ] ISC-494: the Fikih doorway appears on questions that span two areas. **NOT MET — pre-existing, deliberate, not introduced here.** `fiqhAreaOf` returns `null` on a TIE, so `hukum menceraikan istri saat haid` matches `menceraikan` (talak, n=1) and `haid` (thaharah, n=1) and names no area — on a question that is plainly about talak. The conservatism is correct in kind (it declines to guess rather than guessing wrong) but costs the doorway exactly where a reader most needs it. Changing tie-breaking touches the re-rank the whole Fikih safety argument rests on, so it is not being changed unilaterally.
