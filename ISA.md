...
- [ ] ISC-419: an authored answer never hand-writes a translation of scripture. **FIXED AT THE INGESTION POINT 2026-08-12; NOT MET until deployed and re-measured.** Rule 2 said only *"you do not need to quote the translation yourself"* — an invitation, not a rule, which is exactly why the model quoted anyway. Now a prohibition: *"NEVER write out the translation of an ayah yourself — not in quotation marks, not as 'yang artinya', not as a paraphrase presented as the verse's wording."* Fixed in the PROMPT rather than the wall, deliberately — a hard egress rule would reject the app's best answers and fall back to the caption list Erik refused, whereas a prompt rule cannot cost a good answer. Original evidence, unguarded: The `arabic` rule stops the model writing the Arabic; nothing stops it writing its own Indonesian rendering of an ayah in quotation marks. Live: `bolehkah aku pacaran` shipped *"Dan janganlah kamu mendekati zina; sesungguhnya zina itu adalah suatu perbuatan yang keji dan suatu jalan yang buruk." (QS Al-Isra 17:32)* — a translation the model composed, sitting beside the app's own pinned-corpus translation. `apakah musik haram` shipped one prefaced *"yang artinya kurang lebih"*. The app's entire rights and provenance posture is that translations come from the pinned corpus with attribution.
  ...
  **THREE RESIDUAL LIMITS. Limit 1 is NARROWED — not closed; limit 2 is open and got WORSE; limit 3 is why limit 1 is not closed, and limit 3 is ITSELF NARROWED 2026-08-30 — six of its eight tail shapes closed, the two `dan banyak <T>` shapes declined.**
  ...
  **NARROWED 2026-08-30 ON ERIK'S RULING — SIX OF THE EIGHT TAIL SHAPES CLOSED, TWO DECLINED. And the mechanism was never proximity, which is why limit 1's proposed fix could not reach it.** `APPOSITIVE_BREAK` already carried a preposition list; it rejected a preposition only IMMEDIATELY BEFORE the owner token, and every one of the eight tail shapes puts a word in between — `kepada setiap <T>`, `bagi hamba dan <T>`, `beserta <T> semua`. The guard was blind to prepositions **already in its own vocabulary**. Widened with `beserta|bersama|dalam|antara|atas|tentang` and a `(?:\w+\s){0,2}` gap. **`72 of 72` HAS BEEN REPLACED BY A MEASUREMENT**: `src/eval/tail-position-grid.ts` (`bun src/eval/tail-position-grid.ts`) scores 8 shapes × 15 current `DIVINE_VERB` verbs × 26 owner tokens = 3,120 rows, and reports **540 → 2,040 refused, with `divine_attr` going from firing ZERO times across every tail row to 1,950.** Six shapes close completely, 0 → 390 each. ⚠ **THE OLD "NINE LOOSE VERBS" NO LONGER NAME THE TREE** — six of them moved INTO `DIVINE_VERB` on 2026-08-29, and a verb outside `DIVINE_VERB` opens every row trivially because no divine arm can fire at all, so a grid built on the old nine measures the vocabulary gap that was CLOSED rather than the tail-position gap that was open. **THE TWO `dan banyak <T>` SHAPES REMAIN OPEN BY DESIGN** — `dan` is a coordinate conjunction that genuinely can introduce a new subject (`…, dan kita menegaskan, "…"`), so closing them would swallow the one case this arm must not. That residue is declined, kept here with its failing strings, and **still not pinned by a passing test.**

  ⚠ **THE PRICE, MEASURED NOT ESTIMATED: a human owner introduced by a preposition now over-refuses.** `bersama para sahabat Imam Syafii menuturkan, "…"` and `dalam pengajiannya ustadz kami mengatakan, "…"` PASS on HEAD and REFUSE on tree — **+2 over-refusals on 4 clean rows.** The cost probe was 8 rows and **4 of them were CONFOUNDED and are excluded**: they contained `Nabi`, which trips the `prophetic` wall on BOTH arms, so they could not witness this change in either direction. Quoting 6 of 8 as the cost would have been a denominator mixing rows that cannot fail with rows that can — the error this criterion corrects elsewhere. Taken deliberately in the same direction as the 2026-08-29 divine-verb ruling: an under-refusal is the app printing its own rendering as Allah's words, an over-refusal is a quotation declined.

  ⚠ **`bun test` WAS 2425/0 WITH AND WITHOUT THIS CHANGE**, so the suite was blind in both directions and only the paired grids saw behaviour move — the second consecutive day this wall has produced that result. ⚠ **THE PREPOSITION LIST IS DUPLICATED IN `OTHER_AGENT` (`answer-guard.ts` ~line 438) AND WAS DELIBERATELY NOT WIDENED THERE**; it answers a different question. That duplication bit during this very change: a scripted edit anchored on the shared list hit `OTHER_AGENT` FIRST and left `APPOSITIVE_BREAK` untouched, and only a readback of the target line caught it. **ISC-419 STAYS `[ ]`** — it is gated on deploy and re-measure, and prod is unchanged.
