...
**Fix:** (a) pin the pronoun to the user's own register — if the incoming message uses `aku`/`gue`, the reply uses `aku`/`kamu`, never `Anda` in the emotional lane. (b) Extend the receipt rule already applied to the Prophet's words: every sentence making a claim about God, the unseen, or a ruling carries a reference or is cut. (c) Move the AI disclaimer from the foot of the thread to a chip at the head of the composed paragraph, at the same size and weight as the `Terjemahan makna` chip — a label on the prose, not a footnote after it.
