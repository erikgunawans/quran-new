...
const GROUPS: [string, RegExp, string][] = [
  ["Surface & ink", /^--(bg|surface|surface-2|line|line-strong|ink|ink-2|ink-3|panel-ink|panel-ink-2|shell-bg|panel-foot)$/, "The light register. Dark flips only this axis — see `@media (prefers-color-scheme: dark)` in the stylesheet. `--panel-ink` is the exception that proves the rule: it is for type painted on shell.css's panel ground, which flips on the `data-theme` ATTRIBUTE rather than `prefers-color-scheme`, so it deliberately does NOT move with the media query."],
