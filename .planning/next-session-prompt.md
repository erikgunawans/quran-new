...
> `origin/main` reads as the wrap checkpoint, not `207463e`. That is EXPECTED, not drift — the
...
> **Supersedes the Cycle 19 block below**, whose §2.1 (chase the intro-panel ghost) and §2.3
> (correct ISC-659's cost) are ANSWERED, and whose framing of the pane collapse is now FALSIFIED.

Resume New-Quranku — read `PROGRESS.md` first (top checkpoint **2026-08-29, Cycle 20**).

## 0. ⚠️ WHAT CHANGED THAT INVALIDATES OLD METHOD

- **THE `data-pane="text"` COLLAPSE IS NOT AN UNSUMMONABLE TRIGGER. IT IS A READER CLICKING
  "Teks Surah".** Two handoffs recorded it as the part of the trigger nobody could summon, and a
  session was nearly spent waiting for it. `read.ts` writes `data-pane` in exactly ONE place — the
  delegated click handler in `bindSplit` — with no resize observer, timer, or visibility handler.
  Caught live: `{click, trusted:true, closestTab:true}` then `{attr, split→text}` 6 ms later.
  **Summon it by clicking the tab.** Do not wait for it, and do not describe it as mysterious.
- **THE DOCUMENTED FIFTEEN-MINUTE RECIPE DOES NOT REPRODUCE AT 35–39 MINUTES.** Ten arm-runs across
  two sits, both pane states, prod as shipped, with and without the closing scroll: **every arm
  painted, 0 cards at opacity 0, no ghost.** ⚠️ **Do NOT re-run the recipe unchanged and expect a
  different answer** — vary something (machine sleep, memory pressure, a competing heavy tab, a REAL
  pointer scroll rather than a scripted `scrollTop`, a longer sit) or you buy another null.
- **`interceptor screenshot --pixel` FAILS WHEN THE TAB'S *WINDOW* IS NOT OS-FOCUSED**, with the
  misleading *"Extension has not been invoked for the current page (see activeTab permission)"*.
  Erik has ~5 Chrome windows. **Raise the window first** (AppleScript `set index of window i to 1`).
  Same class as the old minimized-window blocker; this is the cause, not a permissions problem.
- **`interceptor eval` RUNS IN AN ISOLATED WORLD** — `window.X` silently does NOT persist between
  calls. Any recorder/state needs `--main`. A recorder that reads back `undefined` is this, not a
  failed install.
- **After `tab switch`, the next `eval` often returns the stale `ok` ack** (response-queue desync).
  Throw away one warmup eval, or re-read. An `ok` where you expected JSON is this.
- **`interceptor tab close/switch <id>` follows the FOCUSED WINDOW's active tab and IGNORES the id.**
  It errored naming Erik's WhatsApp tab. Its guard refused rather than closing his tab, and all 48
  of his tabs were verified intact — but never assume the id was honoured.
- **`interceptor screenshot --save` writes to CWD, which is THE REPO.** Move files out; three landed
  in the repo root this session.
- **A LOADED MACHINE TURNS `bun test` RED WITH NO REGRESSION.** At `uptime` load **44.6** the suite
  returned **exit 1, 2399/1** — `web/src/peta.test.ts` *"attribution — F-2 … no category route renders
  without attribution"* **timed out at 5000 ms**, and the run took **254.8 s vs 24.5 s** on the same
  tree. Green again at 61 s once load eased; the file alone is 25/25 in 2.05 s. ⚠️ **An F-2
  ATTRIBUTION test going red is not automatically a rights regression** — run `uptime` and re-run the
  file in isolation BEFORE investigating.
- **STILL TRUE from Cycle 19:** `innerText` is a broken instrument on prod (`content-visibility:auto`
  makes it report ~194/200 blank on a page that paints perfectly), and the DEFAULT `screenshot`
  re-renders from the DOM and repaints the artifact you are hunting.

## 1. STATE

ISA **687/701** (680 `[x]` · 7 `[~]` · 14 `[ ]`) — **UNCHANGED. No checkbox moved.** ISC-659 stays
`[~]`: a null result is not a fix, and neither was the reproduction before it.
...
**PROD IS UNCHANGED THIS CYCLE — nothing was deployed, because nothing was fixed.** Worker still
`0d33ef55-800a-4924-8523-895312ad1e1e`, `EDITION="synthesis"`. Prod and `main` in sync.

Clean tree except untracked `WARP.md` — **leave it, never `git add -A`.** Work lands directly on
`main`; there is no open PR and none is expected.
...
1. **THE BLANK-CARD REPRODUCTION NEEDS A NEW VARIABLE, NOT ANOTHER SIT.** The state is real (Erik saw
   it; it was reproduced once in Cycle 19) but 35–39 minutes backgrounded no longer summons it.
   ⚠️ **Do not spend another session on the same recipe.** Untried variables, cheapest first:
   **let the machine SLEEP** with the tab open (Cycle 19's reproduction may have straddled a sleep);
   add **memory pressure** (several heavy tabs); use a **real pointer scroll** rather than
   `scrollTop =` (a scripted scroll may not exercise the compositor path); sit **hours, not minutes**.
   Erik's own report was "after the page has been SITTING" with no stated bound.

2. **TWO ARMS ARE BUILT AND MEASURE NOTHING UNTIL A CONTROL FAILS.** `animation: none` on `.verse`
   (kills the `qkin` opacity-0 entrance) and `content-visibility: visible` (reverts ISC-659) were
   both run for 37+ minutes and both painted — **but so did the positive control**, so neither is
   tested and NEITHER IS CLEARED. ⚠️ **Get the control to fail FIRST.** Re-running these against a
   healthy control buys nothing.

3. **THE ISC-659 PREVENTION QUESTION IS STILL OPEN** — same blocker as Cycle 19, now for the second
   cycle running. Injecting the rule does not RESCUE a broken page (tested). Whether it PREVENTS the
   state is untested, and cannot be tested until item 1 lands.

4. **TWO MEASURED LEADS worth pursuing on their own merits, independent of the bug.**
   (a) All 200 `article.verse` carry `backdrop-filter: blur(18px)` behind a **94%-opaque** fill, and
   disabling it moves the page by **≤3/255 in any channel**. That is 200 composited blur readbacks
   for nothing visible. ⚠️ Before deleting: the same selector styles verse cards in CHAT, over a
   different ground — **re-measure there**, and scope was one surah/theme/viewport.
   (b) `@keyframes qkin { from { opacity: 0 } }` runs on every card, and `shell.css` already warns it
   "could strand a card invisible if the tab backgrounded first."

5. **ISC-486, ISC-323, ISC-487 unchanged.** 486's proper-name vocabulary still NOT BUILT. 323 needs a
   DESIGN decision. 487 NOT MET. Other open: ISC-98, 417, 419, 420, 440.6, 454, 464, 566, 627.7,
   654, 353.0. **ISC-419 is still a NULL RESULT** — reproducer
   `apa yang al quran katakan tentang neraka` legitimately returns **0 themes**; do not read its 0 as
   a classifier fault.

## 3. Open items waiting on Erik

- 🔶 **`gcloud auth application-default login`** — creds written 2026-08-26 08:26 but the token now
  fails *"Reauthentication failed, cannot prompt during non-interactive execution"*. Needs ONE more
  interactive run. **Play button stays dead (`audioUrl: null`) until then.** Quota project inside is
  still `story-maker-demo`. **Raised again this cycle, still not done.**
- 🔶 **`launchctl bootstrap gui/$(id -u) ~/Library/LaunchAgents/ai.axiara.quranku.kajian-runner.plist`**
  — written and inert. ⚠️ Erik's 2026-08-28 ruling removed the per-item human gate that was
  incidentally the pipeline's ONLY cost ceiling. **There is still no per-day cap.**
- 🔶 **The two review-pack discrepancies** (`eee00c72`): it says *"tidak ada nama penceramah yang kami
  ...
  `Belum diperiksa` while the slide/audio carry `belum boleh diposting`. **NOT edited — his approved
  document.**
...
- 🔶 **`MEMORY.md` TRADE AUTHORIZATION — now FOUR unwritten traps and still zero headroom.** Cycle 20
  adds: `--pixel` needs the tab's WINDOW focused; `eval` is isolated-world without `--main`;
  `tab close <id>` ignores the id. Plus Cycle 19's two and Cycle 18's cap-distribution one.
  ⚠️ Mechanical trimming and merging were TRIED AND REVERTED; read `CLAUDE.md` first.
- 🔶 **Three of my tabs were left OPEN and could not be closed** (interceptor followed his focused
  window): `893422594` and `893422597` carry **INJECTED NON-PRODUCTION CSS** — do not read them as
  prod. Also `893422600` and an `about:blank`. (Cycle 19's "did Erik lose tabs?" is moot — Chrome
  had restarted and all 48 of his tabs were verified present.)
...
- 🔴 **The blank-card TRIGGER is STILL not understood, and is now HARDER than last cycle thought.**
  The pane collapse is explained (a click), but that was never the whole trigger — collapsing the
  rail deliberately and sitting 39 minutes reproduces nothing.
- 🔴 **Arms C and D are UNTESTED, not cleared.** Anyone reading Cycle 20's results table must not
  conclude `qkin` or `content-visibility` was eliminated.
- 🔴 **The published slide contradicts itself BY ERIK'S EXPLICIT RULING** — carries *"DRAFT — belum
  ...
  and DECLINED 2026-08-28. **Do NOT "fix" this** — "as-is" was chosen OVER "soften".
  See `docs/review/erik-ruling-2026-08-28-kajian-publish-scope.md`.
...
- **NEW — a state read taken ACROSS a reveal can straddle a human click.** Erik clicks tabs you
  foreground in a window in front of him; six trusted clicks were logged this cycle. One pair of
  reads briefly looked like a DOM/pixel mismatch and was NOT one. **Read the recorder first.**
- **NEW — verify a "backgrounded N minutes" claim from the page's OWN `visibilitychange` stamps**,
  not from wall-clock intent. Setup churn re-foregrounds tabs.
- **NEW — never diagnose paint with `innerText` or `getBoundingClientRect`.** Both report a healthy
  page mid-defect, and `innerText` now reports a broken one on a healthy page.
- **NEW — a paired run whose CONTROL did not fail is not evidence.** Said last cycle, and it cost
  this cycle's headline result anyway. Ask what the FAILING arm returned before believing a clean one.
- **Erik's 2026-08-28 kajian ruling is a PUBLISHING rule and does NOT reach ISC-630's SOURCING hold.**
  That channel stays off the queue; its capture stays deleted.
- **A withdrawal that lives only in a commit message is not a withdrawal.**
...
- **Gate commands must REDIRECT, never pipe into `head`/`tail`** — the preflight hook blocks it,
  because `$?` becomes the pipe's status and a failing gate reads as a pass.
...
> `git fetch` + `git ls-remote`, never a push pipe. **Supersedes the 2026-08-24 morning block
> below**, whose "nothing is deployed" line is now FALSE.

Resume New-Quranku — read `PROGRESS.md` first (top checkpoint **2026-08-24 close**).

## 0. ⚠️ PROD IS DEPLOYED. DO NOT REPEAT THE OLD "NOTHING IS DEPLOYED" LINE.

Live Worker **`8634ed83-9036-4f5e-9422-85bc5ef85dc0`**, deployed 2026-08-23T23:48Z, superseding
`641f8ae2`. It shipped **22 commits / 6,402 insertions** — the entire backlog from `44ed447`. The
range was diffed first: no scripture, answer or hadith surface changed. Verified live afterwards
(routes, gates, `#/masuk`, `#/surah/1`, `#/kajian`).

**A deploy now ships only what is new since `c425e1e`** — but `git diff <live>..HEAD` before the
next one anyway; that habit is what caught this range being 22 commits deep.

## 1. STATE

**D1 EXISTS:** `new-quranku-memory`, `caa07809-f6d4-4d8e-92bb-98400f3e08de`, region APAC, all four
migrations applied to REMOTE and verified by reading the schema. `DB` **is bound**; `KAJIAN` is not,
and the bucket `new-quranku-kajian` **does not exist**.

Gates at the anchor: `bun test` **2213/0** exit 0 · typecheck exit 0 · synthesis build exit 0 ·
`wrangler deploy --dry-run` exit 0, bindings now **DB + VECTORIZE + AUDIO + CORPUS** — **run
locally; this repo has no CI.**
ISA **667/683** — 662 `[x]` + 5 `[~]`, **16 open**, counted by grep. **No `### Phase` headers, no
`.planning/STATUS.md`, no `sm-update`** — no phase table and no gap tracker exist. **The 16 open
ISCs ARE the open-item list:** ISC-98, 323, 353.0, 417, 419, 420, 440.6, 454, 464, 486, 487, 533,
534, 552, 566, 627.7. The 5 partial: ISC-418, 617, 624, 624.8, 627.
...
1. **The answer-wall cluster** — the largest untouched group: ISC-533, ISC-534 (the `answer-blocked`
   copy and its anti-downgrade pair), ISC-454 (live `bad_hadith` rate on prod — **and prod is now
   current, so this probe finally measures the shipped code**), ISC-487 (the wall's cost is latency:
   retrieval is 1.2-2.7s of a 5.5-27.3s turn, and the **12s client abort is the real defect**),
   ISC-486, ISC-552. Read `ISA.md` for each first.

2. **ISC-323** (live-Worker hadith-muslim-154 probe) is newly worth running for the same reason —
   prod is no longer months behind the tree.

3. **The other open ISCs, none started:** ISC-98 (real-device visualViewport), ISC-419/420
   (translation + scholar-receipt at ingestion), ISC-440.6 (two pinned Qur'anic-narrative
   sentences), ISC-464 (two P0s from the 2026-08-15 critique), ISC-353.0 (superseded, kept for the
   trail), ISC-627.7 (what deliberately remains).

4. **ISC-566 and ISC-417 stay OPEN by Erik's decision.** Do not "fix" them.

## 3. Open items waiting on Erik

- 🔶 **Three secrets — sign-in is one step from working.** `IDENTITY_HMAC_SECRET`,
  `RESEND_API_KEY`, `ADMIN_EMAILS` (erik@axiara.ai). **Hidden prompt only; NO REDEPLOY NEEDED** —
  secrets take effect immediately.
  ⚠️ **Tell him again when he does it:** `IDENTITY_HMAC_SECRET` is the switch that starts
  PERSISTING THE TEXT OF EVERY QUESTION (`index.ts:671`), not the DB binding. Sign-in and question
  logging turn on together.
- 🔶 **Which source does the runner launch on?** Needs a first video with clear rights. **Nothing
  can be published until this is answered.**
- 🔶 **Nobody has read a repaired answer for THEOLOGICAL correctness.** Needs Ustadz Ahmad.
- 🔶 **No real `short.m4a` has ever been written** — no ADC on this machine
  (`gcloud auth application-default print-access-token` exits 1). Needs
  `gcloud auth application-default login`, interactive, Erik's.
- 🔶 **The runner half:** `RUNNER_SECRET` + creating the `new-quranku-kajian` bucket + uncommenting
  `[[r2_buckets]]`. Until then the admin queue accepts jobs nothing can drain.
- Unchanged and older: a correction with no delivery path · Answer Record retention · roster entries
  (`docs/kajian/roster.yaml` ships EMPTY on purpose).
...
- **The admin queue accepts jobs NOTHING CAN DRAIN.** A deliberate departure from ISC-617, accepted
  by Erik 2026-08-24 so sign-in could work. A queued job sits `queued` indefinitely.
- **ISC-624.8's remainder:** (a) no real `short.m4a`; (b) **no `speechSynthesis` fallback**, because
  a feed record carries **no summary TEXT** — a browser voice could only read the TITLE. Needs the
  bullets in `index.json`: a DATA decision. (c) **No ground-truth pixel of the native audio
  control** — the DOM render does not paint UA shadow content.
- **The hosted runner has never run.** `/kajian/index.json` is now a REAL route returning
  `{"items":[]}` — no longer the SPA fallback. The LOCAL pipeline (`src/app/kajian.ts`) has run.
  **Do not conflate them.**
- **Logout does not REVOKE** — `Max-Age=0`, the value stays valid for its remaining 30 days. The
  shared `canonical_user_id` defect is **NOT fixed** (ADR 2 scopes it out). `splitSentences` is dead
  except its own tests, kept deliberately.
- **`.scratch/kajian/jNQXAC9IVRw/` ("Me at the zoo", 19s) is RETAINED** as the pipeline's only test
  capture — third-party material too, recorded rather than glossed.
- The ISA denominator counts checkbox LINES; ~10 ids are duplicated, pre-existing.
...
- **NEW — a deploy ships the whole range, and this one was 22 commits deep.** `git diff <live>..HEAD`
  and read the commit list BEFORE deploying, every time. Prod had silently fallen months behind.
- **NEW — `wrangler d1 migrations list` does not print in sorted order.** It showed 0001, 0003,
  0004, 0002. Read the SQL for real dependencies (`ALTER TABLE`, `REFERENCES`) rather than trusting
  the display. **0003 BEFORE 0004** is the one that bites.
- **NEW — verify a migration by reading the schema, not the ✅.** `sqlite_master` +
  `pragma_table_info`, not the status table wrangler prints.
- **NEW — a binding and its secret are different switches.** Binding `DB` changed nothing a reader
  can observe; `IDENTITY_HMAC_SECRET` is what starts persisting questions. State the consequence
  next to the cause, in the config.
- **A fixture in the wrong SHAPE is worse than no fixture** — absolute-url fixtures kept the suite
  green while EVERY published kajian card rendered as `""`. Copy values from the PRODUCER.
- **A guard line that can never fire looks exactly like one that works** — mutate to find which line
  is load-bearing; reading cannot tell you.
- **Verify the page runs the build on disk BEFORE believing any browser measurement.** A stale
  bundle made a correct fix read as broken.
- **`interceptor screenshot` cannot paint UA shadow DOM** (native media controls), and the OS
  capture grabs Chrome's FRONT window, which `tab switch` cannot move across windows. Substitute
  computed-style probes and **state the pixel gap**.
- **"Darussalam" is TWO ENTITIES** — the PUBLISHER (a real translation credit) and *dār as-salām*
  (Qur'anic). Read every hit before replacing any.
- **`admin` and `reviewer` are DISJOINT, not a ladder.** `requireRole` matches EXACTLY (ADR 4).
- **The runner is a SECOND AUTH PRINCIPAL.** Never reach for `requireRole` on `/api/runner/*`.
- **Count the ISA with grep, never from expectation.** **Measure before you delete.**
- **The slide's stylesheet lives in a TEMPLATE LITERAL** grepped whole by the force-red test — no
  backtick, and no length or colour literal even inside a COMMENT.
- **The slide budget is bound by the WRAPPED LINE COUNT.** At 1920x1080: 497 chars fits, 581
  overflows by ~48px. Re-render and LOOK.
- **Headless Chrome clamps at ~500 CSS px then CROPS.** Probe narrow layouts at ≥520 or via CDP.
- **Deleting a string from the working tree does not remove it from a public repo.** A history
  rewrite is DECLINED (ISC-639) — do not re-offer it.
- **`bun run typecheck | tail` hides exit 2.** Redirect, echo `$?`, then read. A pre-tool hook blocks
...
> verify with `git fetch` + `git ls-remote`, never a push pipe. **Supersedes the 2026-08-23 (night)
> block below.** From that handoff: item 0/1 (the PRD + ISC-624 slide) **BUILT — see §0**; item 2
> (VPS runner) **untouched**; item 3 (D1+Resend) **untouched and still correct**; item 4 (kajian step
> 7) **still unspecified, still needs Erik**; item 5 (ISC-566) **still open by Erik's decision**.

Resume New-Quranku — read `PROGRESS.md` first (top checkpoint **2026-08-23 session close**).

## 0. WHAT CHANGED, SO YOU DO NOT REPEAT THE LAST HANDOFF'S §0

**THE LETTER IS CANCELLED. Erik, 2026-08-23:** *"let's forget about any letter. i hereby cancel all
requirements to send letter whatsoever."* The previous handoff's first open item — *"Read and send
the letter"* — **is dead. Do not reinstate it, do not draft a replacement, do not treat any sentence
in it as a live promise.** `docs/review/surat-darussalam-2026-08-23.md` is retained as an ARCHIVE
with a cancellation banner; the body is kept only so the record shows what stopped.

**The landscape slide is BUILT** (ISC-624.1..7, .9). **Darussalam's material is out of the code**
(ISC-627.1..6). Neither is deployed and neither is reader-facing.

## 1. STATE

Prod: `new-quranku.axiara.ai` → Worker **`641f8ae2` from `44ed447`** — unchanged since 2026-08-23
morning. **Nothing in this session is deployed**; the slide is a build-time CLI artefact, not a route.

Gates at the anchor: `bun test` **2048/0** exit 0 (25,926 assertions) · typecheck exit 0 ·
`VITE_ANSWER_MODE=synthesis bun run build` exit 0 — **run locally; this repo has no CI.**
ISA **651/671** (648 `[x]` + 3 `[~]`, 20 open), counted by grep. **`ISA.md` has no `### Phase`
headers and there is no `.planning/STATUS.md`** — so no phase table and no gap tracker exist to
derive. **The 20 open ISCs ARE the open-item list.**
...
0. **READ `.scratch/kajian-summarize/PRD.md` FIRST**, and note its hold section is **SUPERSEDED** —
   it now points at ISC-630 instead of at the letter. Erik's feature: the Kajian page gets a **URL
   field and a Summarize button**, admin-only, results published to the public list, built against
   the **VPS runner**. Items 1-2 below are the parts of that one job.

1. **The VPS runner + D1/Resend in the SAME change (ISC-617).** Erik picked a VPS knowing it carries
   a datacentre IP, so `yt-dlp` needs exported cookies or a residential proxy, and a fetch failure
   must surface as a JOB STATE, not a silent empty summary. **The runner-facing endpoints are a
   SECOND AUTH PRINCIPAL** — a shared bearer secret, no email, no role, no cookie. Do NOT reach for
   `requireRole`: an Administrator's 30-day `__Host-qk_auth` in a VPS env var undoes ISC-568 entirely.
   Until this lands, the admin route's 403 to everyone — Erik included — is CORRECT (ISC-592). Apply
   migration `0003_kajian_jobs.sql`; it has never touched a real D1.
   ⚠️ **ISC-630 gates what this may PUBLISH, not whether it may be built.** Build it; ask Erik before
   the first Darussalam-sourced summary goes public.

2. **ISC-624.8 — the play button.** `kajian.ts:532` nests the SHORT narration inside
   `if (!NO_VIDEO && …)` and its WAV is consumed only by `stillVideo`, so turning the video off
   destroyed the button's only audio producer. Decouple it, encode it as its own artefact, store it
   (R2 — the `AUDIO` bucket exists for recitation, keyed `{surah}/{ayah}.mp3`, so kajian needs its own
   prefix), and fall back to browser `speechSynthesis` only BEHIND a pre-generated file.
   ⚠️ **Shipping speechSynthesis-only is not a partial delivery — it is the option ADR 6 already
   refused**, because the voice would vary per device. ADR 6's voice is LOAD-BEARING.

3. **The answer-wall cluster**, the largest untouched group: ISC-533, ISC-534 (the `answer-blocked`
   copy and its anti-downgrade pair), ISC-454 (live `bad_hadith` rate on prod), ISC-487 (the wall's
   cost is latency, and the 12s client abort is the real defect — retrieval is only 1.2-2.7s of a
   5.5-27.3s turn), ISC-486, ISC-552. Read `ISA.md` for each before touching any.

4. **Kajian step 7 — still undefined anywhere in the repo.** Asked four times. Ask before starting.

5. **ISC-566 cross-paragraph danglers and ISC-417 stay OPEN by Erik's decision.** Do not "fix" them.

## 3. Open items waiting on Erik

- 🚨 **ISC-630 — the publish hold on Darussalam-sourced summaries has no stated basis any more.**
  Its only justification was a sentence in the cancelled letter. **The DA must not resolve this in
  either direction:** keeping it invents a fresh justification for a rule whose old one was deleted;
  dropping it authorises publishing a third party's material. Facts he needs: the rights position is
  UNCHANGED (~12.7 MB across two `.scratch/` directories, no permission) and never depended on the
  letter; and **nothing is blocked today** because the publish path does not exist. **Needed before
  the runner ships, not before the next commit.** Raise this first.
- **The cancelled letter file** — kept as an archive. He may want it deleted outright; offered, unanswered.
- **ISC-627.7b** — `docs/kajian/roster.yaml` and `kajian-narration.ts` still name real dakwah
  channels in PROSE (as the naming-convention argument, not as fixtures). He answered about the
  FIXTURE; the comments were left deliberately. Offered, unanswered.
- **ISC-627.7d** — git history still holds every scrubbed string (13 pushed commits, earliest
  `43eee9e`). A rewrite is **offered, not assumed** — it changes every SHA, including ones the deploy
  records cite.
- **Kajian step 7** — undefined. **"Maruli"** — the string appears NOWHERE in the repo; asked twice,
  never answered; do not guess.
- **Rights items 1 and 2** (does the logo clause reach a summary; carry their logo or not) — his
  alone. Item 3 ("ask the mosque?") is now MOOT, since no letter is going.
- **ISC-626** — recount the `briefing.md` quotations from the directories, prefer the larger reading.
- Unchanged and older: a correction with no delivery path · nobody has read a repaired answer for
  THEOLOGICAL correctness · Answer Record retention · ISC-554's remaining half · roster entries.

## 4. Known weaknesses — recorded, NOT fixed

- **`docs/kajian/roster.yaml` is EMPTY** (`speakers: []`). Deliberate: no slide names anyone.
- **~12.7 MB of Darussalam material in gitignored `.scratch/kajian/`, across TWO directories.**
  Unpublished, unpublishable without permission, and now governed by ISC-630 rather than by a letter.
...
- **`kajian_jobs` is unmigrated on any real D1.** **Logout does not REVOKE.** **The shared
  `canonical_user_id` defect is NOT fixed** (ADR 2 scopes it out). **`splitSentences` is dead** except
  its own tests, kept deliberately.
- The ISA denominator counts checkbox LINES; ~10 ids are duplicated, pre-existing.
...
- **NEW — the slide's stylesheet lives in a TEMPLATE LITERAL and is grepped whole by the force-red
  test.** No backtick anywhere in it (it ends the string, and tsc then points at the prose), and **no
  length or colour literal even inside a COMMENT** — `999px` and `500px` in comments each turned the
  suite red. Spell numbers out. See `token-guard-reads-comments`.
- **NEW — the slide budget is bound by the WRAPPED LINE COUNT, not the character count.** 395 and 497
  chars render the same eight lines. Bracket, measured at 1920x1080: **497 fits with ~28px spare, 581
  overflows by ~48px.** Re-render the real briefing AND a ceiling case and LOOK; never raise it by
  arithmetic. `overflow: hidden` was removed on purpose — the page grows so a break is visible.
- **NEW — headless Chrome clamps its own window at about five hundred CSS pixels and then CROPS the
  screenshot to the width you asked for.** A 412-wide shot showed every card cut and it was an
  artifact. **The tell: the cut falls at the same x on unrelated elements.** Probe narrow layouts at
  ≥520, or via CDP `emulate`.
- **NEW — renaming a fixture is a mutation test.** Scrubbing the kajian fixtures exposed three
  assertions that could never fail, and the first fix for one was *itself* trivially true. Derive
  expectations from the fixture, then PROVE the derived version reddens.
- **NEW — deleting a string from the working tree does not remove it from a public repo.** Run
  `git log -S"<string>" --all` before promising anything about removal.
- **NEVER batch a file write with a gate command in one Bash call** — a blocked hook DISCARDS EVERY
  WRITE in that call. Hit TWICE this session, both times by a `| head`/`| tail` inside the same call.
- **Never pipe a gate command into `head`/`tail`** (the hook blocks it); never pipe `git push`.
  Verify a push with `git ls-remote`, which is the only arbiter.
- **Always `VITE_ANSWER_MODE=synthesis bun run build`** — a bare `bun run build` produces a
  `principled` dist and deploying it silently un-authors prod. Check `.build-meta.json` first.
- **A deploy ships the whole range, not your change.** `git log <deployed>..HEAD -- worker/` before
  deploying, and say what else is going out. **`wrangler deploy` uploads the DIRECTORY** and has never
  read `.gitignore`; on this origin a 200 proves nothing (SPA fallback) — compare BYTE COUNT and type.
- **Re-gate after applying fixes, with a CHANGED FRAME** — audit the corrections, not the originals.
...
- **Never `str.replace` into a record without asserting the target exists** (an assert fired this
  session and correctly aborted). **Arabic you type is not the Arabic that shipped** — splice bytes.
- **The kajian tool never rewrites a transcript** and never copies credentials from a video title.
- ⚠️ **ADR 6's voice IS load-bearing.** ⚠️ **ADR 5 survives every redesign** — every slide says it is
  an automatic summary and not a quotation, and `roster.yaml` is empty so no slide names anyone.
...
> `git fetch` and `git ls-remote`, never a push pipe. **Supersedes the earlier 2026-08-23 block.**
> From that handoff: item 1 (where the runner lives) **ANSWERED — a VPS**. Item 2 (the admin route)
> **DONE**. Item 3 (kajian step 7) **still unspecified, still needs Erik**. Item 4 (ISC-566 danglers)
> **still OPEN by Erik's decision — do not "fix" them.**

Resume New-Quranku — read `PROGRESS.md` first (top checkpoint **2026-08-23 (late)**).

## 0. THE ONE THING THAT MATTERS MOST

**Two unnamings are committed and NOT DEPLOYED.** Until Erik deploys, `new-quranku.axiara.ai` still
shows Ustadz Ahmad Isrofiel's name in two places the 2026-08-23 rulings removed it from — the Hadits
permission banner and the surah preface. **This is the only work in the repo where "committed" and
"fixed" genuinely differ.** Deploy is gated to Erik; do not deploy, and do not describe these as done.

## 1. STATE

Prod: `new-quranku.axiara.ai` runs Worker **`2b7707f2`** from `4a144ad`. **Nothing this session was
deployed.**

Gates at the anchor: `bun test` **2017/0** exit 0 · typecheck exit 0 · build exit 0 — **run locally;
this repo has no CI**, so no automated verifier attests them. ISA **624/641**.

Clean tree except untracked `WARP.md` — **leave it, never `git add -A`.**

**The ustadz position:** ISC-417 stays `[ ]` PERMANENTLY. ⚠️ Never write a blanket *"the ustadz has
approved nothing"* — it is FALSE and voids three real permissions (F-1 2026-07-17; co-display
2026-07-23; machine hadith Indonesian as-is 2026-08-12, VERBAL and relayed, hadith TEXT layer only).
Scope to *"in reply to these two letters"*. Anything not explicitly released is **HELD BY DEFAULT**.
**The two unnamings do NOT withdraw the hadith permission** — the permission claim stays, only the
name went.

## 2. DO NEXT, IN ORDER

1. **Ask Erik to deploy**, or confirm he is deliberately holding. The unnamings are the reason. Also
   ask whether he wants the Darussalam letter sent, and whether to disclose in it that a derivative
   mp4 already exists locally (his call, wording is in `docs/review/surat-darussalam-2026-08-23.md`).
2. **The VPS runner.** Erik picked a VPS knowing it carries a datacentre IP, so `yt-dlp` will need
   exported cookies or a residential proxy. **The runner-facing endpoints are a SECOND AUTH
   PRINCIPAL** — a shared bearer secret, no email, no role, no cookie. Do NOT reach for `requireRole`:
   putting an Administrator's 30-day `__Host-qk_auth` in a VPS env var undoes ISC-568 entirely.
3. **Bind D1 + Resend on prod IN THE SAME CHANGE as the runner** (ISC-617, Erik's decision). Until
   then the admin route's 403 to everyone — Erik included — is CORRECT and must not be "fixed". The
   reason is ISC-592, not just an unset `ADMIN_EMAILS`.
4. **Kajian step 7 — still undefined anywhere in the repo.** Ask what it is before starting.
5. **ISC-566 cross-paragraph danglers stay OPEN by Erik's decision.** Do not "fix" them.

## 3. Open items waiting on Erik

- **Deploy** (item 1) — the unnamings are inert until then.
- **Send the Darussalam letter?** Drafted and unsent; the DA never sends. Plus the one judgment left
  to him inside it.
- **Kajian step 7** — undefined.
- **"Maruli"** — Erik asked to be shown "the trick kajian that is maruli". The string appears NOWHERE
  in the repo; the one real video's speaker is **Ustadz Syariful Mahya**. Asked once, never answered.
  Ask again before guessing.
...
- **The unnamings are undeployed** (§0).
- **A derivative mp4 and slide from Darussalam's video sit in gitignored `.scratch/kajian/`**, built
  2026-08-22. Unpublished, and unpublishable without their permission.
- **The hosted runner does not exist** — nothing writes `/kajian/index.json`, so `#/kajian` shows its
  empty state. The LOCAL runner (`src/app/kajian.ts`, 535 lines) does exist and has run; do not
  conflate the two, a previous session did and put it in three documents.
- **The queue has no consumer**, so its row shape is validated only by our own tests.
- **`kajian_jobs` is unmigrated on any real D1.** Migration 0003 has never been applied anywhere.
- **Logout does not REVOKE**; the only lever is bumping `SESSION_DOMAIN`.
- **The shared-`canonical_user_id` defect is NOT fixed** — ADR 2 scopes it out.
- **`splitSentences` is dead** except its own tests. Kept deliberately.
- The ISA denominator counts checkbox LINES; ~10 ids are duplicated, pre-existing.
...
- **NEW — never `str.replace` into a record without asserting the target exists.** Two ISA edits
  silently no-oped in a multi-replace script that reported success; a gate found them. With asserts,
  the next run ABORTED on a genuine miss and wrote nothing, which is the behaviour wanted.
- **NEW — measure probe numbers LAST, and re-measure after every later edit.** Counts drifted
  ×12/×8/×3 → ×16/×9/×5 → ×17/×9/×7 in one session; each was true when written, false when read.
- **NEW — a correction can overshoot into a falsehood that RETIRES A REAL GUARD.** "That sentence
  appears nowhere" would have told the next developer that a live, tested rule was invented.
- **NEW — a green suite does not mark an ISA.** 17 criteria sat `[ ]` for hours after passing; the
  wrap's per-cycle table found them, and one of them turned out not actually met.
- **NEW — check the whole repo for a bad citation, not just the docs.** The worst carrier of the
  ISC-10 error was a source docblock, and it was the most-read of the three.
- **A scholarly-gate BLOCK on a CORRECTION is the norm.** Re-gate after applying fixes, always, and
  give the re-gate a CHANGED FRAME — auditing the corrections, not re-reading the originals.
- **Do not enumerate which tests a defect affects unless you just measured it.**
- **A test signed at a fixed clock against code calling `Date.now()` passes VACUOUSLY.**
- **`/api/answer` cannot be probed by curl** — drive the app with Interceptor, clear
  `localStorage['newquranku:thread']`, discard a warm-up turn.
- **Never batch a file write with a gate command** — a blocked hook DISCARDS EVERY WRITE in that call.
  Hit again this session by a `grep | head` inside a gate call.
- **Never pipe a gate command into `head`/`tail`**; never pipe `git push`. Verify with `git ls-remote`.
- **Arabic you type is not the Arabic that shipped.** Splice captured bytes.
- **Do not edit files while an auditing agent is auditing them.**
- **The kajian tool never rewrites a transcript** and never copies credentials from a video title.
- ⚠️ **ADR 6's voice IS load-bearing again** — Erik's 2026-08-23 four-questions ruling KEEPS
  `id-ID-Chirp3-HD-Schedar` and reverses the voice half of his own skill-wins ruling. The
  speaker-naming half of skill-wins STANDS.
- ⚠️ **Kajian ruling (b) is REFUSED PERMANENTLY** — a metadata speaker name may be WRITTEN, never
  SPOKEN. Pinned by tests in `src/app/kajian-speaker.test.ts`, including a runtime assertion that
  survives a `RosterOutcome` widening tsc would allow.
...
> `git fetch`, not by a push pipe). **Supersedes the `874578d` anchor.** From that handoff: item 1
> (`MEMORY.md` oversize) **DONE**. Item 2 (cross-paragraph danglers) **SCOPED, not fixed — and the
> scoping falsified the obvious fix**. Item 3 (ISC-561's live measurement) **DECLINED BY ERIK, and
> recorded as declined rather than pending**. Items 4 (Track B step 1) and 5 (kajian step 7) were not
> started and carry forward; both need Erik's pick.

Resume New-Quranku — read `PROGRESS.md` first (top checkpoint **2026-08-22 (late night)**).

## 0. CORRECTION THIS BLOCK IS OBLIGED TO CARRY — read before quoting any dangler evidence

**Every handoff block below this one, and the `PROGRESS.md` checkpoint below the newest, describe the
dangler witness WRONGLY.** They say the reader got *"a first paragraph opening"* on *"Selain itu,"*
and *"a close"* / *"a closing"* on the `sungai` phrase. Against
`docs/review/captures/api-answer-9ab57d4b-2026-08-22.json` turn-3, the truth is:

- *"Selain itu,"* opens the surviving **SECOND** paragraph. The surviving **first** is the greeting
  *"Tentu, pertanyaan yang sangat baik…"*. This is what LOCATES the excision.
- The sentence *"Jadi, setiap kali Anda merasa berat atau malas untuk shalat, ingatlah sungai rahmat
  yang mengalir itu."* **OPENS** the surviving **THIRD** paragraph — the `sungai` phrase itself ENDS
  that opening sentence — and **two sentences follow it**. It does NOT close the answer.

ISC-566 supersedes both stale records by pointer; neither is edited in place because both files are
append/prepend-only. **This block IS the vehicle. Do not defer it again.**

## 1. STATE — prod unchanged this session, both commits are records

`new-quranku.axiara.ai` runs Worker **`2b7707f2`** (`EDITION: "synthesis"`), built from `4a144ad`.
**No code shipped this session** — `5373f54` and `10a7ac0` are both ISA records; the only `.ts` change
was inside a docblock. Nothing is on fire.

Gates at the anchor: `bun test` **1859/0** exit 0 · typecheck exit 0 · build exit 0. ISA **574/590**
(573 `[x]` + ISC-418 `[~]`; 15 open, 1 deferred). Clean tree except untracked `WARP.md` — **leave it,
never `git add -A`.**

**The ustadz position is UNCHANGED from the previous block and still governs.** Erik withdrew the
WAIT (2026-08-22, `docs/review/erik-ruling-2026-08-22.md`) — schedule only, not provenance, and no
promise released. **ISC-417 stays `[ ]` PERMANENTLY.** ⚠️ Never write a blanket *"the ustadz has
approved nothing"* — it is FALSE and voids three real permissions (F-1 2026-07-17; co-display
2026-07-23; machine hadith Indonesian as-is 2026-08-12, VERBAL and relayed, hadith TEXT layer only).
Always scope to *"in reply to these two letters"*. Every promise in either letter is PERMANENT, and
**anything not explicitly released is HELD BY DEFAULT** — the authoritative sets are each letter's
§*Yang tidak kami lakukan* plus every *"selama menunggu"* clause; no summary is the boundary.

## 2. DO NEXT, IN ORDER

1. **Cross-paragraph danglers — ISC-566 is SCOPED and waiting on ERIK'S COST DECISION.** Do not write
   code first. The entry records four candidate fixes and declines all four; the recommendation is no
   code fix at this granularity. What Erik must choose: a thinner answer (connective list, or
   prefix-only truncation), a real coherence component beside the rules wall, or the hole as it
   stands. **The cheapest fix is already falsified** — cascading to the connective-opening successor
   drops the paragraph holding that answer's whole evidence AND leaves the second dangler untouched.
2. **If he wants evidence before deciding, build the DIAGNOSTIC, not a guard.** Over already-captured
   repaired turns, flag any surviving paragraph that opens on a back-referring connective whose
   predecessor was excised, or that carries a definite noun phrase occurring nowhere else in the
   surviving prose. It ships nothing and only counts. It must SHARE the repair's binding, never copy
   it (`diagnostic-outlives-its-gate`). Note its own limit: it cannot separate an antecedent-less
   definite from an ordinary first mention, which is exactly why it counts and never guards.
3. **Track B step 1, if Erik picks it** — the `qk_auth` signed cookie + `roleFor()`.
4. **Kajian step 7, if Erik wants it** — steps 1-6 complete, step 7 still unspecified. Ask what it is
   before starting; nothing in the repo defines it.

## 3. Open items waiting on Erik (unchanged unless noted)

- **NEW — which cost to pay for the dangler class** (item 1 above). Nothing moves until he says.
- **THE THREE KAJIAN RULINGS, still owed** — (a) the channel name printed on the slide while the
  narration refuses to speak it, (b) model-relayed speaker names being spoken, (c) consensus claims
  unscreened in the autoplay mp4. **(a) and (b) land physically on the slide.**
- **The Hadits banner still names the ustadz on a VERBAL relay basis**, and the written confirmation
  asked for will never come. The letter's own offer — replace with an unnamed sentence, or remove it
  — **stands open and is Erik's.**
- **A correction with no delivery path.** A note bound the NEXT LETTER to carry specific corrections;
  Erik decided (2026-08-21) not to send a third letter. An undelivered correction must not quietly
  become one that never existed.
- **The permanently-held promises** — each needs an explicit, item-by-item ruling to release.
- **Nobody has read a repaired answer for THEOLOGICAL correctness.** Four read for coherence only.
- **Kajian slide visual design was never firmed** — fixed-height body box leaves ~⅓ empty at four
  bullets, URL breaks mid-string on `word-break: break-all`, DRAFT band wraps to two lines.
- **Answer Record retention**, **ISC-554's remaining half**, **roster entries** — unchanged.

## 4. Known weaknesses — recorded, NOT fixed

- **Cross-paragraph danglers** — scoped in ISC-566, open. Two forms at two distances.
- **A single-paragraph answer that trips the wall ships SILENCE** (ISC-564 cost (b)).
- **Three or more offenders reporting the SAME `detail` still end in silence** (ISC-562's bound).
- **ISC-565's frame covers only the TWO WITNESSED nouns.** Widen only from real transcripts.
- **ISC-561's widening is unwitnessed LIVE and the measurement is declined, not pending.** The
  mechanism is unit-pinned; do not re-open this as if it were untested. `refusal-capture.ts` cannot
  settle it — the hadith lane is absent on EVERY turn there.
- `splitSentences` is **dead** except for its own tests. Kept deliberately.

## 5. Where the kajian criteria live — STILL NOT IN `ISA.md`

Step 4's 38 ISCs are in a TASK ISA at `~/.claude/PAI/MEMORY/WORK/kajian-slide-qr/ISA.md`. Steps 5-6
have NO task ISA. **`quran-new/ISA.md` has zero kajian criteria — never report 574/590 as kajian
progress.**

---

## Constraints to honor (carried forward — plus two new)

- **NEW — record a retraction ONLY where a reader can still encounter the false claim.** ISC-566 took
  SIX gate passes and nearly every finding after the first was a defect the previous pass's own
  corrections introduced. Each *"a first version said X"* sentence is a factual claim about document
  history that can be wrong; inside a NEW entry it points at text in no git object, so it can only
  ever be wrong. Seven were cut and no fact was lost — one had become false. Memory:
  `retraction-needs-a-reachable-reader`.
- **NEW — `repairedAttempt` indexes the REFUSED candidates, not `attempts`.** `attempts` also collects
  `threw`, `empty` and `ok`, so `repairedAttempt: 0` with `attempts.length: 2` does NOT prove the
  ISC-561 widening fired. **Count the `blocked:` rows.**
- **A `scholarly-gate` BLOCK is almost always the RECORD, not the code** — the code was clean from the
  first pass in every change so far. **ALWAYS re-gate after applying its fixes.**
- **Do not accept an auditing agent's measurement either.** Re-measure in both directions.
- **`/api/answer` cannot be probed by curl** — drive the app with Interceptor, clear
  `localStorage['newquranku:thread']`, and discard a warm-up turn.
- **Never batch a file write with a gate command** — a blocked hook DISCARDS EVERY WRITE in that Bash
  call. Hit again this session: a trailing `| tail` blocked a whole command and nothing ran.
- **Never pipe a gate command into `head`/`tail`** — `$?` becomes the pipe's status. Redirect, then
  echo the exit code.
- **A green wall is not a readable answer.** The guard is a RULES wall, not a coherence check.
- **Arabic you type is not the Arabic that shipped.** Splice captured bytes; paste witnesses from the
  capture, never retype them.
...
> Prepended by /wrap 2026-08-22 (night). **Anchor `874578d` on origin/main** (verified by
> `git fetch`, not by a push pipe). **Supersedes the `cc7f5df` anchor.** From that handoff: item 1
> (third gate pass on ISC-565) **DONE — and it took four passes, not one**. Item 2 (deploy ISC-565)
> **DONE — DEPLOYED AND VERIFIED LIVE**, because Erik withdrew the ustadz gate. Item 4 (measure
> ISC-561) **got one data point, still unwitnessed**. Items 3 (cross-paragraph danglers), 5
> (`MEMORY.md` oversize), 6 (Track B step 1) and 7 (kajian step 7) were NOT started and carry
> forward. The old §2's ustadz items are **RESOLVED BY RULING** — see §0.

Resume New-Quranku — read `PROGRESS.md` first (top checkpoint **2026-08-22 (night)**).

## 0. STATE — prod is healthy, and the ustadz gate is GONE

`new-quranku.axiara.ai` runs Worker **`2b7707f2-0dbf-4f1c-83cf-7400a5d0ffce`** (`EDITION:
"synthesis"`), built from `4a144ad`. **ISC-564 and ISC-565 are both live.** Nothing is on fire.

**ERIK WITHDREW THE WAIT ON THE USTADZ (2026-08-22).** Recorded at
`docs/review/erik-ruling-2026-08-22.md` — read it before touching anything in `docs/review/`.
**It changed the SCHEDULE, not the PROVENANCE.** Ustadz Ahmad has sent nothing in reply to the two
letters and never will be waited on again. **ISC-417 stays `[ ]` PERMANENTLY — never re-mark it
`[x]`.** The app ships AI-authored religious answers with NO scholarly sign-off, by Erik's knowing
decision, so the reader-facing disclosures are now MORE load-bearing, not less.

⚠️ **Never write a blanket "the ustadz has approved nothing".** It is FALSE and voids three real
permissions: F-1 = yes (2026-07-17), co-display (2026-07-23), machine hadith Indonesian as-is
(2026-08-12 — VERBAL, relayed; hadith TEXT layer only, NOT bab titles, and on our own narrow
reading the Hadits page only, NOT the answer card). Always scope to "in reply to these two letters".

⚠️ **Every PROMISE inside those letters is now PERMANENT, not lapsed.** The binding sets are each
letter's §*Yang tidak kami lakukan* (four bullets and seven) plus every *"selama menunggu"* clause
in their bodies. **Anything not explicitly released is HELD BY DEFAULT.** Held today: the God/unseen
filter (ISC-464(b)), the hukum-nikah/hukum-waris pinned lists, hadith ORDERING unchanged on
technical grounds, and `MAX_DISPLAY = 2` (separately Erik's own ruling in `rights-2026-08-20.md` §4
— never reachable by a bookkeeping edit).

Gates at the anchor: `bun test` **1859/0** exit 0 · typecheck exit 0 · build exit 0. ISA **574/589**
(573 `[x]` + ISC-418 `[~]`; 14 open, 1 deferred). Clean tree except untracked `WARP.md` — **leave
it, never `git add -A`.**

## 1. DO NEXT, IN ORDER

1. **`MEMORY.md` is over its 24.4 KB limit and only PART OF IT LOADS.** Silent context loss at every
   session start, carried across three handoffs now. Shorten index lines to <200 chars or fold old
   ones into topic files. Cheap, and it degrades every future session until done.
2. **Cross-paragraph danglers — ISC-564's cost (c), WITNESSED LIVE and unscoped.** Capture at
   `docs/review/captures/api-answer-9ab57d4b-2026-08-22.json`: a reader got a first paragraph opening
   **"Selain itu,"** and a close pointing at a parable the excision removed. **It did NOT recur** on
   the 2026-08-22 night verification turn (three coherent paragraphs, `repairedDropped: 2`) — one
   turn is evidence, not a class. Nobody has scoped the fix.
3. **ISC-561's widening is STILL unwitnessed.** `gen.repairedAttempt: 0` has not appeared in any of
   the THREE live repairs captured. A measurement job needing many turns, not a fix.
4. **Track B step 1, if Erik picks it** — the `qk_auth` signed cookie + `roleFor()`.
5. **Kajian step 7, if Erik wants it** — steps 1-6 complete, step 7 unspecified.
...
- **THE THREE KAJIAN RULINGS, still owed** — (a) the channel name printed on the slide while the
  ...
  unscreened in the autoplay mp4. **(a) and (b) land physically on the slide.**
- **The Hadits banner still names the ustadz on a VERBAL relay basis**, and the written confirmation
  requested will never come. The letter's own offer — replace with an unnamed sentence, or remove it
  — **stands open and is Erik's.** Separate from the exception he retired.
- **A correction with no delivery path.** A note bound the NEXT LETTER to carry specific corrections;
  Erik decided (2026-08-21) not to send a third letter. Ending the wait removed the vehicle. An
  undelivered correction must not quietly become one that never existed.
- **The four permanently-held promises** in §0 — each needs an explicit, item-by-item ruling to
  release.
- **Nobody has read a repaired answer for THEOLOGICAL correctness.** Four have now been read for
  coherence only.
- **Kajian slide visual design was never firmed.** Functionally finished, visually a default: a
  fixed-height body box leaves ~⅓ of the slide empty at four bullets and ~⅔ on the empty state; the
  URL breaks mid-string on `word-break: break-all`; the DRAFT band wraps to two lines.
- **Answer Record retention**, **ISC-554's remaining half**, **roster entries** — unchanged.

## 3. Known weaknesses — recorded, NOT fixed

- **Cross-paragraph danglers** — witnessed live, twice in one answer. See §1 item 2.
- **A single-paragraph answer that trips the wall ships SILENCE**, and ISC-565 makes the wall fire on
  strictly more shapes.
- **Three or more offenders reporting the SAME `detail` still end in silence** (ISC-562's bound).
- **ISC-565's frame covers only the TWO WITNESSED nouns** (`perumpamaan`, `gambaran`). Widen only
  from real transcripts — the second surface form appeared within HOURS of the first.
- **The `di-` arm of `LIGHT_VERB_SPEECH` is narrower than it looks** — verb-then-noun order only, so
  it does NOT reach "gambaran yang diberikan Rasulullah ﷺ". Kept because widening only adds refusals.
- `splitSentences` is **dead** except for its own tests. Kept deliberately.

## 4. Where the kajian criteria live — STILL NOT IN `ISA.md`

Step 4's 38 ISCs are in a TASK ISA at `~/.claude/PAI/MEMORY/WORK/kajian-slide-qr/ISA.md`. Steps 5-6
have NO task ISA. **`quran-new/ISA.md` has zero kajian criteria — never report 574/589 as kajian
progress.**
...
- **NEW — `fd` is NOT on PATH in this shell and fails SILENTLY, returning empty.** A directory search
  reported "no kajian files" and was wrong; only cross-checking with `ls` caught it. **Use `find`.**
- **NEW — a closed list under a heading claiming totality IS the same bug as "one thing".** Two
  successive gate BLOCKs on exactly this: "one promise survives" → a closed four-item list headed
  "every promise", which still omitted six. **The fix is a POINTER to the authoritative set plus
  "anything unlisted is held by default"** — never an enumeration presented as the boundary.
- **NEW — cite by SECTION NAME, never line number, in `docs/review/`.** Four line numbers were cited
  and the same edit had already invalidated all four. `ustadz-followup-2026-08-18.md` states the rule
  outright, because the numbers had already shifted three times — each time by a note written
  alongside its own pointer.
- **NEW — retiring an exception RAISES the stakes on the promise.** It becomes unconditional, so the
  code satisfying it must be pinned. `web/src/hadith-permission-notice.test.ts` exists for that;
  force-red it (revert to the pre-`6d4d909` string → 3 of 4 fail) before trusting it.
- **NEW — `.build-meta.json` can be STALE ABOUT ITS OWN CONTENTS.** It recorded `2f23edd` while
  `web/dist` already held `cc7f5df`'s guard. Necessary evidence, NOT sufficient — confirm by the
  inlined literal or a content probe.
- **A `scholarly-gate` BLOCK is almost always the RECORD, not the code** — TWELVE passes across four
  changes now, code clean from the first every time. **ALWAYS re-gate after applying its fixes:**
  four of this session's findings were defects a previous fix introduced.
- **Do not accept an auditing agent's measurement either.** Re-measure in both directions; two of its
  claims this session needed my own scan to confirm, and our whole-tree totals disagreed.
- **`/api/answer` cannot be probed by curl** — no hash-verified grounding means it bails before
  ...
  `localStorage['newquranku:thread']`, and discard a warm-up turn.
- **Never batch a file write with a gate command** — a blocked hook DISCARDS EVERY WRITE in that Bash
  call, and the untouched baseline then reads as a green new suite.
- **Never pipe a gate command into `head`/`tail`** — `$?` becomes the pipe's status. Redirect, then
  echo the exit code. (The preflight hook blocks this, correctly.)
- **A green wall is not a readable answer.** The guard is a RULES wall, not a coherence check.
- **Arabic you type is not the Arabic that shipped.** Splice captured bytes; run a codepoint scan
  over the diff.
- **The granularity was never Erik's.** His ruling is *"it has to be answered"*.
- **Do not edit files while an auditing agent is auditing them.**
- **The kajian tool never rewrites a transcript**, widens its cue list ONLY from real transcripts, and
