...
- **The devotional-app cliché — which is ornament, not green.** Arabesque wallpaper, crescent moons, mosque-dome silhouettes, gold filigree frames, calligraphy-as-decoration. The failure is ornament used as a substitute for reverence: decoration standing in for the thing it decorates.
  ...
  **On the celestial ground, revised 2026-07-18.** New-Quranku takes the night sky as its ground. The Qur'an calls its own verses *āyāt* — signs — the same word it gives the sun, the moon, and the alternation of night and day (*"and it is He who made the stars for you, that you might be guided by them"*, 6:97). A restrained field of stars, a single crescent, an emerald-to-indigo sky, and the green→gold signature carried into that ambient background are **atmosphere, not filigree** — the reader can no more mistake weather for wallpaper than mistake a dark sky for a decorated card. So the ban's two last edges are redrawn deliberately: **the crescent and the night sky are permitted as the app's celestial ground, and gold is permitted in that ambient background** — the same signature it earns in the hero type. What stays banned is unchanged and exact: gold, or any ornament, **on the content**. No gold frame, rule, hairline, or edge on a card; no crescent used as a badge, bullet, or divider; no filigree tracing anything the reader reads. The line is where it always was — ornament as a substitute for reverence — now drawn firmly around the *content*, leaving the *ground* free to be sky. (The peta cosmos data-viz encoding is a separate rule, ISC-185, and is not touched here.)

  **One functional exception on content, same date.** The surah index tags each surah **Makkiyah** or **Madaniyah** — QuranKu's own semantic pair, **gold for Meccan, green for Medinan**. This is gold carrying *information*, not decorating a card: a reader learns something true from the colour. It is therefore permitted as a **functional region tag** and nowhere else on content — it is not licence for gold to creep onto card edges, rules, or headings. The gold is a literal value tuned to pass AA (a darker amber on light, the signature gold on dark), never an oklch palette token. Decorative gold on content stays banned exactly as above.
