...
Nur never arbitrates between scholars. It shows you that Ibn Kathir and As-Sa'di read this differently, names them both, and trusts you. Plurality is warmth, not hedging.

## Anti-references

Two reflexes, both rejected:

- **The devotional-app cliché.** Emerald-and-gold, arabesque wallpaper, crescent moons, mosque-dome silhouettes, gold filigree frames, calligraphy-as-decoration. Guessable from the category alone. Ornament used as a substitute for reverence.
...
1. **Light emerging from dark.** *Nūr* is the Qur'an's metaphor for itself (24:35). The word is the light source; the interface is the room around it. Design the room dark enough that the text glows — and bright enough, by day, that it sings.
...
5. **Never fabricate; silence is honest.** Where the corpus is silent, Nur is silent. It shows the verse and the sources and says plainly that it doesn't know. A Qur'an app that confabulates is worse than no app.
